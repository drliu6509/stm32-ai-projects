...
static lv_res_t list_release_handler(lv_obj_t * list_obj)
{
    ...
    if(lv_indev_get_type(indev) == LV_INDEV_TYPE_POINTER || lv_indev_get_type(indev) == LV_INDEV_TYPE_BUTTON) {
        /*修复LVGL已知bug(#4605): 释放时lv_indev_get_point()返回的坐标可能已失效(为0),
         * 导致get_id_on_point()计算出越界索引而被截断为最后一项.
         * 因此改用按下时(list_press_handler)保存的pr_opt_id, 该索引基于有效坐标计算.*/
        if(dropdown->pr_opt_id < dropdown->option_cnt) {
            dropdown->sel_opt_id     = dropdown->pr_opt_id;
            dropdown->sel_opt_id_orig = dropdown->sel_opt_id;
        }
    }
