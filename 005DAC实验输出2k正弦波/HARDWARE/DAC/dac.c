...
u16 DAC_WAVE_VAL[DAC_WAVE_POINT_NUM]; //正弦波数据表

//生成正弦波数据表
//参数:无
//返回值:无
static void DAC1_Sin_Table(void)
{
    u16 i;
    for(i=0;i<DAC_WAVE_POINT_NUM;i++)
        DAC_WAVE_VAL[i]=(u16)((sin(2*3.1415926f*i/DAC_WAVE_POINT_NUM)+1)*2048);   //幅值0~4095,对应0~3.3V
}

//初始化DAC1:TIM6触发+DMA输出2K正弦波
//DMA循环模式自动把正弦波数据表送到DAC,无需CPU干预
void DAC1_Init(void)
{
    DAC_ChannelConfTypeDef DACCH1_Config;
    TIM_MasterConfigTypeDef sMasterConfig;
    
    DAC1_Sin_Table();                    //生成正弦波数据表
    ...
    DAC_DMA_Handler.Init.Mode=DMA_CIRCULAR;         //循环模式,正弦波自动循环输出
