...
MP3DecInfo *AllocateBuffers(void)
{
  ...
  ClearBuffer((void*)&mp3DecInfo, sizeof(MP3DecInfo));
   ...
   // return mp3DecInfo_pointer;
	...
	mp3DecInfo_pointer = (MP3DecInfo *)mymalloc(SRAMEX,sizeof(MP3DecInfo));
	if (!mp3DecInfo_pointer)
		return 0;
	ClearBuffer(mp3DecInfo_pointer, sizeof(MP3DecInfo));
	
	fh =  (FrameHeader *)     mymalloc(SRAMEX,sizeof(FrameHeader));
	si =  (SideInfo *)        mymalloc(SRAMEX,sizeof(SideInfo));
	sfi = (ScaleFactorInfo *) mymalloc(SRAMEX,sizeof(ScaleFactorInfo));
	hi =  (HuffmanInfo *)     mymalloc(SRAMEX,sizeof(HuffmanInfo));
	di =  (DequantInfo *)     mymalloc(SRAMEX,sizeof(DequantInfo));
	mi =  (IMDCTInfo *)       mymalloc(SRAMEX,sizeof(IMDCTInfo));
	sbi = (SubbandInfo *)     mymalloc(SRAMEX,sizeof(SubbandInfo));
...
#define SAFE_FREE(x)	{if (x)	myfree(SRAMEX,x);	(x) = 0;}	/* helper macro */
#endif
